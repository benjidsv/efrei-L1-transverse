from math import pi, radians

import pygame as pg

# config
RES = WIDTH, HEIGHT = 1280, 720
MENU_FPS = 24
GAME_FPS = 60

# game settings
GOAL_WIDTH, GOAL_HEIGHT = 637, 212
GOAL_X, GOAL_Y, GOAL_Z = 321, 140, 482

GOALKEEPER_WIDTH, GOALKEEPER_HEIGHT = 45, 145
GOALKEEPER_START_X, GOALKEEPER_START_Y = WIDTH / 2 - GOALKEEPER_WIDTH/2, GOAL_Y + GOAL_HEIGHT - GOALKEEPER_HEIGHT
GOALKEEPER_SPEED = 6
GOALKEEPER_JUMP_SPEED_X, GOALKEEPER_JUMP_SPEED_Y = 520, 850
GOALKEEPER_VERTICAL_JUMP_SPEED = 0.7
GOALKEEPER_JUMP_ANGLE = radians(20)
GOALKEEPER_TURN_SPEED = 2.5

BALL_START_WIDTH, BALL_START_HEIGHT = 26, 26
BALL_START_X, BALL_START_Y, BALL_START_Z = WIDTH/2 - BALL_START_WIDTH/2, 490 - BALL_START_HEIGHT/2, 0
BALL_END_WIDTH, BALL_END_HEIGHT = 20, 20
BALL_DEPTH_TO_SIZE = (BALL_END_WIDTH - BALL_START_WIDTH) / GOAL_Z
BALL_DEPTH_TO_HEIGHT = (GOAL_Y + GOAL_HEIGHT - BALL_START_Y) / GOAL_Z
BALL_TARGET_IF_FLOOR_SHOT = 350
BALL_TIME_S_TO_STRENTGH = 100
BALL_SPEED_FACTOR = 1.6
MAX_SHOOT_HOLD_TIME = 1
MAX_STRENGTH = (MAX_SHOOT_HOLD_TIME * BALL_TIME_S_TO_STRENTGH)
STRENGTH_TO_HEIGHT_ADJUSTMENT = (-160 - BALL_TARGET_IF_FLOOR_SHOT) / MAX_STRENGTH
GRAVITY = 1500

FIELD_WIDTH, FIELD_HEIGHT = WIDTH, 416
FLOOR_LINE = pg.Vector3(0, GOAL_Y - BALL_START_Y + GOAL_HEIGHT, GOAL_Z - BALL_START_Z)

MAP_WIDTH, MAP_HEIGHT = 320, 180
MAP_PADDING = 32
MAP_X, MAP_Y = WIDTH - MAP_WIDTH - MAP_PADDING, HEIGHT - MAP_HEIGHT - MAP_PADDING
MAP_FLOOR_HEIGHT = 12
MAP_GOAL_X, MAP_GOAL_Y = MAP_X + 300, MAP_Y + MAP_HEIGHT - 54
MAP_BALL_START_X, MAP_BALL_START_Y = MAP_X + 80, MAP_Y + MAP_HEIGHT - 23
MAP_BALL_SIZE = 8
MAP_BORDER_WIDTH = 6
MAP_TRAJECTORY_STEP = 1
MAP_BALL_GOAL_DISTANCE = MAP_WIDTH - 60

# labels
MS_TO_S = 0.001
PI_HALVES = pi/2
OPTIMAL_WIDTH_FACTOR = 0.02

# sprites
GOALKEEPER_SPRITE = pg.image.load("assets/goalkeeper.png")
BALL_SPRITE = pg.image.load("assets/ball.png")
MAP_SPRITE = pg.image.load("assets/map.png")

# colors
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
GREEN = (65, 235, 90)
BLUE = (125, 210, 255)
PINK = (255, 100, 100)


